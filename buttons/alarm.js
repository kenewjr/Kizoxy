...
async function showAlarmList(interaction, client, userId, statusMsg = "") {
  ...
  if (alarms.length === 0) {
    return interaction.editReply({
      content: statusMsg || "❌ Anda tidak memiliki alarm aktif.",
      embeds: [],
      components: [],
    });
  }
  ...
  execute: async (interaction, client) => {
    ...
    try {
      ...
      if (action === "alarm_cancel_select") {
        ...
        if (!alarms.length)
          return interaction.editReply({
            content: "❌ Tidak ada alarm.",
            embeds: [],
            components: [],
          });
        return interaction.editReply({
          content: "🗑️ **Pilih alarm yang ingin dibatalkan:**",
          embeds: [],
          components: [
            buildAlarmSelect(alarms, "alarm_cancel_do"),
            buildBackButton(),
          ],
        });
      }
      ...
      if (action === "alarm_cancel_do") {
        ...
        return showAlarmList(
          ...
          client,
          userId,
          `✅ Alarm **"${alarm.message}"** berhasil dibatalkan!`,
        );
      }
      ...
      if (action === "alarm_edit_select") {
        ...
        return interaction.editReply({
          content: "✏️ **Pilih alarm yang ingin diedit:**",
          embeds: [],
          components: [
            buildAlarmSelect(alarms, "alarm_edit_show"),
            buildBackButton(),
          ],
        });
      }
      ...
      if (action === "alarm_toggle_select") {
        ...
        return interaction.editReply({
          content: "⏯️ **Pilih alarm untuk di-toggle:**",
          embeds: [],
          components: [buildAlarmToggleSelect(alarms), buildBackButton()],
        });
      }
      ...
      if (action === "alarm_toggle_do") {
        ...
        if (result.alarm.userId !== userId)
          return interaction.editReply({
            content: "❌ Alarm bukan milik Anda.",
            components: [],
          });
      ...
      try {
        await interaction.editReply({
          content: "❌ Terjadi error. Silakan coba lagi.",
          components: [],
        });
      } catch (_) {}
    }
