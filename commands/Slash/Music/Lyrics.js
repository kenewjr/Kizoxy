  ...
  run: async (client, interaction) => {
    ...
    try {
      ...
      if (!voiceChannel || voiceChannel.id !== player.voiceId)
        return interaction.editReply({ content: "❌ You must be in the same voice channel as the bot." });

      const track = player.queue?.current;
      if (!track)
        return interaction.editReply({ content: "❌ No track is currently loaded." });
      ...
      if (!fullLyrics?.trim())
        return interaction.editReply({ content: "🔹 Lyrics are empty or unavailable for this track." });
