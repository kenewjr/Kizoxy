...
(async () => {
  ...
  try {
    ...
    console.log(
      "Current git commit: (error reading git commit:",
      e.message + ")",
    );
  }
  ...
  try {
    console.log(
      "Uncommitted changes:",
      execSync("git status --porcelain").toString().trim() || "(none)",
    );
  } catch (e) {
    console.log(
      "Uncommitted changes: (error checking git status:",
      e.message + ")",
    );
  }
  ...
  const clientPath = path.resolve(
    __dirname,
    "../src/integrations/tiktok/client.js",
  );
  try {
    console.log("client.js last modified:", fs.statSync(clientPath).mtime);
  } catch (e) {
    console.log(
      "client.js last modified: (error checking stat:",
      e.message + ")",
    );
  }
  ...
  for (const username of testUsernames) {
    ...
    try {
      const res = await fetch(
        `https://www.tikwm.com/api/user/posts?unique_id=${encodeURIComponent(username)}&count=12&cursor=0&web=1&hd=1`,
        {
    ...
    try {
      const {
        loadCookies,
      } = require("../src/integrations/tiktok/cookieStorage");
      const cookies = loadCookies();
      const hasCookies = Boolean(
        cookies &&
        (Array.isArray(cookies) ? cookies.length > 0 : Boolean(cookies)),
      );
      console.log(
        `Cookies configured: ${hasCookies ? "yes" : "NO — this may be the issue if direct scraping is the fallback"}`,
      );
      ...
      const text = await res.text();
      console.log(
        `Direct HTML: HTTP ${res.status}, body length ${text.length}`,
      );
      console.log(
        `Contains rehydration script: ${text.includes("__UNIVERSAL_DATA_FOR_REHYDRATION__")}`,
      );
      console.log(`Contains 'itemList': ${text.includes("itemList")}`);
      console.log(
        `Contains captcha/blocked indicators: ${/captcha|verify|blocked|denied/i.test(text.slice(0, 3000))}`,
      );
    } catch (e) {
    ...
    try {
      ...
      console.log(
        `Profile existence check: HTTP ${res.status} (404/410 = account doesn't exist or was removed)`,
      );
    } catch (e) {
  ...
  for (const username of testUsernames) {
    try {
      ...
      console.log(
        `@${username}: live=${profile.user.live}, videos=${profile.videos.length}`,
      );
    } catch (e) {
